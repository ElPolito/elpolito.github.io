...
export default class ApparitionEffectComponent extends React.Component<IProps, IState> {
  ...
  public render = () => {
    ...
    return (
      <div
        ref={this.state.ref}
        className={`comp-apparition-effect reveal-effect ${effect} ${this.state.revealed ? 'revealed' : ''} ${
          this.props.className
        }`}
      >
        {this.props.effect.length === 1 ? (
          this.props.children
        ) : (
          <ApparitionEffectComponent effect={effects}>{this.props.children}</ApparitionEffectComponent>
        )}
      </div>
    );
  };
